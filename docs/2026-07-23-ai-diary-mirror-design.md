...
> 版本：v1.4（设计修正）
> 日期：2026-07-23（初版） / 2026-08-04（架构更新） / 2026-08-07（审核机制重构） / 2026-08-12（文档与代码对齐） / 2026-08-12（设计修正）
> 状态：设计中，持续完善
...
2. 点击提交
3. Java 后台异步处理：
   - **Java**：长度检测（>500 字提示用户，不阻断）
   - **gRPC → Python AI 服务**：一次调用完成拆分 + 分类（标题/摘要/标签）
   - **Java**：每条分类结果保存为一条 record（status = reviewing）
4. 前端展示审核界面，等待用户操作
5. 用户审核通过后（`confirmReview`）：
   - **gRPC → Python AI 服务**：文本转向量（Embedding）
   - **Java**：保存 chunks 表（pgvector 向量存储）
   - status = done

> **关键设计**：
> - **拆分 + 分类在一次 LLM 调用中完成**：Python 端先判断是否有多件事，有就拆成多条，每条独立分类，一次性返回。不在输入层单独拆分。
> - **Embedding 在用户审核通过之后才执行**，确保 RAG 中存储的向量和元数据都是用户确认的最终版本。
...
**AI 处理示例（多事件拆分）：**
```
用户输入："今天上午学了 Spring Boot，感觉很有收获。
          下午和朋友吃饭聊了毕业设计。晚上有点焦虑因为明天有 deadline。"

AI 处理结果（3 条 record）：
├── record 1:
│   ├── 标题：学习 Spring Boot
│   ├── 类型：learning | 情绪：[satisfied]
│   └── 关键词：Spring Boot, 学习
├── record 2:
│   ├── 标题：和朋友聊毕业设计
│   ├── 类型：social | 情绪：[calm]
...
└── record 3:
    ├── 标题：明天 deadline 焦虑
    ├── 类型：thought | 情绪：[anxious]
    └── 关键词：deadline, 焦虑

每条 record 独立进入审核流程，用户可逐条确认或删除。
```
...
|------|----------|
| ≤ 500 字 | 正常处理 |
| > 500 字 | 前端提示用户（不阻断），AI 仍可处理 |

**特殊情况处理：**

| 场景 | 处理方式 |
|------|----------|
| 多件事混合 | 分类层一次 LLM 调用同时完成拆分 + 分类，返回多条结果，每条生成一条 record |
| 纯情绪输出（如"啊啊啊啊"） | AI 判定 skip=true，跳过分类，记录标记为 failed |
| 超长内容（> 500字） | 前端提示用户，AI 仍可处理（LLM 自行按语义拆分） |

> **设计变更说明**：拆分从"输入层"移至"分类层"。拆分本质是 AI 任务（需要理解语义），与分类在同一次 LLM 调用中完成效率最高（1 次调用 vs N+1 次），且用户只需在最终结果上审核一次。
...
Java：长度检测（>500 字提示用户，不阻断）
    ↓
Java：gRPC 调用 Python AI 服务（Classify）
    └── Python 内部：判断是否需要拆分 → 逐条分类 → 返回多条结果
    ↓
Java：每条结果保存为一条 record（status = reviewing）
    ↓
前端展示审核界面，等待用户操作
    ├── 通过（可先修改标签）→ confirmReview()
    └── 删除 → 软删除，流程结束
    ↓
confirmReview()：
    ├── gRPC → Python：Embedding（用户确认后的最终版本）
    ├── 保存 chunks 表（pgvector）
    └── status = done，记录锁定
```
...
│    - 关键词：3-5 个                      │
│ 特殊处理：                               │
│ - 无意义内容 → skip=true，记录标记 failed │
│ 返回多条结果 → Java 每条生成一条 record   │
│ status=reviewing                         │
└─────────────────┬───────────────────────┘
                  ↓
┌─────────────────────────────────────────┐
...
│ 【用户交互】                              │
│ - 展示 AI 生成结果，用户可修改标签         │
│ - 确认 → 触发 Embedding（confirmReview）  │
│ - 删除 → 软删除，流程结束                 │
│ - 自动审核模式 → 跳过此层，直接 Embedding │
└─────────────────┬───────────────────────┘
                  ↓
┌─────────────────────────────────────────┐
│ 第 5 层：向量化 + 存储（Embed + Store）   │
│ 【confirmReview() 中执行】               │
│ - gRPC → Python：文本转向量              │
│   - 本地：BGE-m3（可配置）               │
...
│ - 记录锁定，不可再修改                    │
│ - 一条记录 = 一个 chunk                  │
│ - Embedding 失败不影响确认（记录仍 DONE） │
└─────────────────────────────────────────┘
```

> **设计变更说明**：
> 1. **输入层精简**：拆分从输入层移至分类层（拆分是 AI 任务，与分类在一次 LLM 调用中完成效率最高）。
> 2. **分类层支持多条返回**：ClassifyResponse 使用 `repeated ClassifyItem`，一条输入可返回多条分类结果。
> 3. **Embedding 从管道移出**：不再作为管道的第 3 步自动执行，改为在 confirmReview() 中手动触发，确保用户确认后才 Embedding。
> 4. **审核层是关键节点**：用户确认（confirmReview）同时触发 Embedding 和状态变更。
...
status = processing（AI 处理中）
    ↓
┌─────────────────────────────────────┐
│ 管道异步执行：Clean → Classify       │
│ Classify 内部：拆分 + 分类一次完成    │
│    ├─ 失败 → status=failed           │
│    └─ 成功 → status=reviewing        │
│       （多事件时生成多条 record）     │
└─────────────────────────────────────┘
...
│ 用户操作                             │
│                                     │
│ 1. 确认通过（confirmReview）         │
│    ├─ Embedding → 存 chunks 表       │
│    └─ status=done，记录锁定          │
│                                     │
│ 2. 删除                              │
│    └─ 软删除（REVIEWING/FAILED 可删）│
└─────────────────────────────────────┘
```
...
| RPC 方法 | 超时时间 | 说明 |
|---|---|---|
| Classify | 180s | LLM 调用（含拆分+分类），耗时较长 |
...
**关键：Embedding 在 confirmReview() 中执行，失败时不影响确认流程（记录仍变为 DONE），向量未写入但记录已锁定。后续可通过重试机制补录向量。**
...
| 2026-08-12 | v1.4 | 设计修正：① 拆分从输入层移至分类层（与分类在一次 LLM 调用中完成，Classify 返回 repeated ClassifyItem）；② Embedding 从管道移出，改为 confirmReview() 中触发（用户确认后才 Embedding）；③ 管道精简为 Clean → Classify 两步；④ 超时设置 Classify 30s→180s；⑤ Embedding 失败不影响确认流程（记录仍 DONE）；⑥ 内容长度限制改为前端提示不阻断 |
